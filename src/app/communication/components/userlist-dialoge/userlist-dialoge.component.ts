import { Component, Inject, OnInit } from '@angular/core';
import { GlobalConstants } from 'src/app/config/GlobalConstants';
import { ToastrService } from 'ngx-toastr';
import { FormArray, FormBuilder, FormGroup, Validators } from '@angular/forms';
import { User } from 'src/app/user/model/user.model';
import { Router } from '@angular/router';
import { UserService } from 'src/app/services/components/user/user.service';
import { EmployeeSearchDto } from 'src/app/user-and-group/models/employee-search.model';
import { NudgeGroupsService } from '../../service/nudgeGroups/nudge-groups.service';
import { MAT_DIALOG_DATA, MatDialogRef } from '@angular/material/dialog';
import { MultipleNudgeUsers } from '../../model/nudgeGroupUsersMultiple';
import { NudgeGroup } from '../../model/nudgeGroup.model';
import { MultipleDeptSubAdmin } from 'src/app/user-and-group/models/deptSubAdminMultiple';
import { DepartmentService } from 'src/app/user-and-group/components/service/department/department.service';
import { SquadronSubAdminService } from 'src/app/user-and-group/components/service/squadronSubAdmin/squadronSubAdmin.service';
import { SquadronAddSubAdmin } from 'src/app/user-and-group/models/squadronAddSubAdmin.model';
import { NudgeUserGroupsService } from '../../service/nudgeUserGroups/nudge-user-groups.service';

@Component({
  selector: 'app-userlist-dialoge',
  templateUrl: './userlist-dialoge.component.html',
  styleUrls: ['./userlist-dialoge.component.css'],
})
export class UserListDialogComponent implements OnInit {
  userList: FormGroup;
  displayedUsers: User[] = [];
  itemsPerPage = 10;
  formSubmitted = false;
  airmen: User[] = [];
  loading: boolean = false;
  error: string | null = null;
  selectData: User[] = [];
  dataUserRecord: User[] = [];
  datafilerecords: User[] = [];

  pageableData: EmployeeSearchDto = {
    page: 0,
    size: GlobalConstants.DEFAULT_PAGE_SIZE,
    orderBy: '',
    employeeName: '',
  };
  canLoadMore: boolean;

  constructor(
    private fb: FormBuilder,
    private nudgeGroupsService: NudgeUserGroupsService,
    private squadronSubAdminService: SquadronSubAdminService,
    private userService: UserService,
    private toaster: ToastrService,
    private router: Router,
    private deptService: DepartmentService,
    public dialogRef: MatDialogRef<UserListDialogComponent>,
    @Inject(MAT_DIALOG_DATA)
    public booldata: { isCommandActivity: boolean; isNudgeGroup: boolean; nudgeData: any; ActivityData: any; isSquadron: boolean; squadronData: any }
  ) {
    this.userList = this.fb.group({
      messageGroupName: ['', Validators.required],
      messageGroupDesc: ['', Validators.required],
      userName: [''],
      selectedUsers: this.fb.array([]),
    });
  }

  ngOnInit(): void {
    this.searchUserNudgeGroup(true);
    this.userList.get('userName')?.valueChanges.subscribe(value => {
      if (value === '') {
        this.searchUserNudgeGroup(true);
      }
    });
  }

  closeDialog(): void {
    this.dialogRef.close();
  }

  // Load more users for pagination
  loadMore(): void {
    this.pageableData.page += 1;
    this.searchUserNudgeGroup(false); // Fixed: Do not reset data
  }

  // Toggle user selection
  toggleSelection(user: User): void {
    const selectedUsers = this.userList.get('selectedUsers') as FormArray;
    const index = selectedUsers.controls.findIndex(control => control.value === user.id);

    if (index >= 0) {
      selectedUsers.removeAt(index);
      this.selectData = this.selectData.filter(u => u.id !== user.id);
    } else {
      selectedUsers.push(this.fb.control(user.id));
      this.selectData.push(user);
    }
  }

  // Check if the user is selected
  isSelected(userId: number): boolean {
    const selectedUsers = this.userList.get('selectedUsers') as FormArray;
    return selectedUsers.controls.some(control => control.value === userId);
  }

  // Check if form is valid
  isFormValid(): boolean {
    return this.userList.valid;
  }

  submitForm(): void {
    const selectedUsers = this.userList.get('selectedUsers') as FormArray;
    if (this.selectData.length === 0) {
      this.toaster.error('Please select at least one user before submitting.', 'Error!');
      return;
    } else {
      if (this.booldata.isCommandActivity) {
        this.addCommandActivitySubAdmin(); // Method for isCommandActivity
      } else if (this.booldata.isNudgeGroup) {
        this.addNudgeGroupUsers();
      } else if (this.booldata.isSquadron) {
        this.addSquadronSubAdmin();
      }
    }
  }
  addSquadronSubAdmin(): void {
    const squadronSubAdmin: SquadronAddSubAdmin = {
      id: '',
      squadronId: this.booldata.squadronData.id,
      userId: this.selectData.map(user => user.id),
      firstName: '',
      lastName: '',
      email: '',
      phoneNumber: '',
      role: '',
      roleId: '',
      profilePicture: '',
      name: '',
      isDelete: 1,
      subAdminId: '',
    };

    this.squadronSubAdminService.saveDeptSubAdmin(squadronSubAdmin).subscribe({
      next: response => {
        this.toaster.success(response.msg || 'Squadron Sub Admin added successfully!', 'Success!');
        this.dialogRef.close();
        this.router.navigate(['/dashboard/userAndgroup/squadrons-subAdmin-view']);
      },
      error: errorResponse => {
        console.error('Error adding Squadron Sub Admin:', errorResponse);
        const errorMessage = errorResponse?.error?.msg || 'Failed to add users!';
        this.toaster.error(errorMessage, 'Error!');
      },
    });
  }

  // Save the Nudge Group
  addNudgeGroupUsers(): void {
    const userCollectionList: MultipleNudgeUsers = {
      messageGroupId: this.booldata.nudgeData.messageGroupId,
      userId: this.selectData.map(user => user.id),
      createdAt: '',
      updatedAt: '',
      createdBy: '',
      updatedBy: '',
      isDelete: 1,
    };

    this.nudgeGroupsService.saveNudgeGroupUsers(userCollectionList).subscribe({
      next: response => {
        this.toaster.success(response.msg || 'Users added successfully!', 'Success!');
        this.dialogRef.close();
        this.router.navigate(['/dashboard/communication/view-nudge-group']);
      },
      error: errorResponse => {
        const errorMessage = errorResponse?.error?.msg || 'Failed to add users!';
        this.toaster.error(errorMessage, 'Error!');
      },
    });
  }

  addCommandActivitySubAdmin() {
    const subAdminCollectionList: MultipleDeptSubAdmin = {
      id: '',
      departmentId: this.booldata.ActivityData.id,
      userId: this.selectData.map(user => user.id),
      firstName: '',
      lastName: '',
      email: '',
      phoneNumber: '',
      role: '',
      roleId: '',
      profilePicture: '',
      name: '',
      isDelete: 1,
      subAdminId: '',
    };

    this.deptService.saveDeptSubAdmin(subAdminCollectionList).subscribe({
      next: response => {
        this.toaster.success(response.msg || 'Sub Admin added successfully!', 'Success!');
        this.dialogRef.close();
        this.router.navigate(['/dashboard/userAndgroup/command-activity-view']);
      },
      error: errorResponse => {
        const errorMessage = errorResponse?.error?.msg || 'Failed to add users!';
        this.toaster.error(errorMessage, 'Error!');
      },
    });
  }

  // Handle checkbox selection for users
  checkBoxValueClicked(event: any, type: string, item: User): void {
    const checkbox = event.target as HTMLInputElement;

    if (type === 'all') {
      const checkboxes = document.querySelectorAll('.insurer-table input[type="checkbox"]') as NodeListOf<HTMLInputElement>;
      checkboxes.forEach(chk => {
        chk.checked = checkbox.checked;
      });

      this.selectData = checkbox.checked ? [...this.datafilerecords] : [];
    } else {
      if (checkbox.checked) {
        this.selectData.push(item);
      } else {
        this.selectData = this.selectData.filter(user => user.id !== item.id);
      }
    }
  }

  // Search Users for Nudge Group
  searchUserNudgeGroup(clearData: boolean): void {
    this.pageableData.employeeName = this.userList.get('userName')?.value || '';

    if (clearData) {
      this.datafilerecords = [];
      this.pageableData.page = 0;
    }
    this.userService.searchAirman(this.pageableData).subscribe({
      next: (responseData: User[]) => {
        this.datafilerecords = [...this.datafilerecords, ...responseData]; // Append new records
        if (responseData.length < this.pageableData.size) {
          this.canLoadMore = false; // Disable the "Load More" button if less than 30
        } else {
          this.canLoadMore = true; // Enable the "Load More" button if exactly 30
        }
      },
      error: error => {
        console.error('Error fetching users:', error);
        this.toaster.error('Unable to load data', 'Error');
      },
    });
  }

  isUserSelected(user: User): boolean {
    return this.selectData.some(selectedUser => selectedUser.id === user.id);
  }
}
